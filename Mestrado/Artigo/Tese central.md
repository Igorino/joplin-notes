...
| **Lapeyrade (2022): raciocínio com ontologias (Prolog/WFS)** | Propõe decisão de NPC via "programação lógica e ontologias hierárquicas". Diz que a abordagem lógica "permite explicação fácil e completa do resultado, ao contrário de técnicas de aprendizado". Útil pra explicar o comportamento ao jogador. | O mais próximo da tese. Mesmos eixos (FSM/BT/Utility/Planning). Ataca NN pela opacidade/autoria. |
| **Studiawan et al. (2018): GOAP em jogo espacial** | Implementa GOAP (Orkin/STRIPS) num real-time tactics. Plano de 3 a 5 nós em < 0,01 s. Em um teste o agente diverge do esperado, e o porquê é rastreável via a função de peso. | **Sustentação técnica**. Traz o primário de GOAP (Orkin) que faltava. O desvio rastreável é exemplo de explicabilidade na prática; a velocidade mostra que GOAP é barato, contrastando com o LLM, não com NN em geral. |
